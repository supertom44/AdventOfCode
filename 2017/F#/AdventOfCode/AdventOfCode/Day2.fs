...
module Day2App =
    ...
    let SolvePart1Exp (input:string)  =
        input.Split([|Environment.NewLine|], StringSplitOptions.None)        
        |> Seq.map (fun i -> i.Split([|' ';'\t'|]) |> Array.map (string >> int))
        |> Seq.sumBy(fun row -> Seq.max row - Seq.min row)

    let SolvePart2Exp (input:string)  =
        let rows = input.Split([|Environment.NewLine|], StringSplitOptions.None)        
                    |> Seq.map (fun i -> i.Split([|' ';'\t'|]) |> Array.map (string >> int))
    ...
    let SolvePart1 (input:string) =
        ...
        input.Split([|Environment.NewLine|], StringSplitOptions.None)        
        |> Seq.map (fun i -> i.Split([|' ';'\t'|]) |> Array.map (string >> int))
        |> Seq.fold (fun acc (x) -> acc + (Array.max x - Array.min x)) 0

    
module Day2Tests = 

    let input = @"1208	412	743	57	1097	53	71	1029	719	133	258	69	1104	373	367	365
...
2 4 6 8"
    ...
    [<Test>]
    let ``then the test input produces then correct output``()  = 
        Assert.That(Day2App.SolvePart1 testInput, Is.EqualTo 18)   

    [<Test>]
    let ``then the input produces then correct output``()  = 
        Assert.That(Day2App.SolvePart1 input, Is.EqualTo 54426)
        
    let test2Input = @"5 9 2 8
9 4 7 3
3 8 6 5"
    
    [<Test>]
    let ``then the test input produces then correct output for part 2``()  = 
        Assert.That(Day2App.SolvePart2Exp test2Input, Is.EqualTo 9)

    [<Test>]
    let ``then the input produces then correct output for part 2``()  = 
        Assert.That(Day2App.SolvePart2Exp input, Is.EqualTo 333)