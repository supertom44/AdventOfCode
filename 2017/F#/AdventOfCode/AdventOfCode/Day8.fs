...
module Day8App =
    ...
    let pattern = @"^(\w+)\s(inc|dec)\s([-|\+]?\d+)\sif\s(\w+)\s(>|<|>=|<=|==|!=)\s([-|\+]?\d+)$"

    let regexMatch p i = Regex.Match(i, p)

    let getRegistarOrDefault (registar: int option) = 
        match registar with 
        | Some x -> x
        | None -> 0

    let applyOperation (instruction:Match) ((registars : Map<string,int>), (values : int List))=
        let conditionRegistar = instruction.Groups.[4].Value
        let conditionRegistarValue = registars |> Map.tryFind conditionRegistar|> getRegistarOrDefault
        let conditionComp = instruction.Groups.[5].Value
        let compValue = int instruction.Groups.[6].Value

        let applyCommand =
            match conditionComp with 
            | ">" -> conditionRegistarValue > compValue
            | ">=" -> conditionRegistarValue >= compValue
            | "<" -> conditionRegistarValue < compValue
            | "<=" -> conditionRegistarValue <= compValue
            | "==" -> conditionRegistarValue = compValue
        ...
        if applyCommand = false
        then (registars, values)
        else
            let commandRegistar = instruction.Groups.[1].Value
            let commandRegistarValue = registars |> Map.tryFind commandRegistar|> getRegistarOrDefault
            let command = instruction.Groups.[2].Value
            let commandValue = int instruction.Groups.[3].Value

            match command with 
            | "inc" -> 
                let value = commandRegistarValue + commandValue
    ...
    let SolvePart1 (input:string) =
        ...
        |> Array.map(regexMatch pattern)
        |> Array.fold(fun reg mat -> applyOperation mat reg ) (Map.empty, List.empty)
        |> fst
        |> Map.toArray
        |> Array.maxBy snd
        |> snd        
...
module Day8Tests =
    ...
    let testInput = @"bc inc 5 if aa > 1
aa inc 1 if bc < 5
cc dec -10 if aa >= 1
cc inc -20 if cc == 10"
    ...
    let ``Given a test input then the correct output is returned`` () = 
        Assert.That(Day8App.SolvePart1 testInput, Is.EqualTo(1))

    [<Test>]
    let ``Given a test input for part 2 then the correct output is returned`` () = 
        Assert.That(Day8App.SolvePart2 testInput, Is.EqualTo(10))

    let puzzleInput = @"ev dec -705 if cag != 2
b dec -195 if ty >= -8
red dec -404 if v > -7
d inc -971 if k >= -9
k inc 303 if k > -8
...
b inc 697 if ty <= -1118"
    ...
    [<Test>]
    let ``Given a puzzle input then the correct output is returned`` () = 
        Assert.That(Day8App.SolvePart1 puzzleInput, Is.EqualTo(5143))

    [<Test>]
    let ``Given a input for part 2 then the correct output is returned`` () = 
        Assert.That(Day8App.SolvePart2 puzzleInput, Is.EqualTo(6209))