...
module Day7App =
    ...
    let SolvePart1 (input:string) = 
        ...
        let result = (bases - nodes) |> Set.toList
        result.[0]


    let splitLine2 (line:string) = 
        let parts = line.Split([|"->"|], StringSplitOptions.None)
        let rawNode = parts.[0].Split(' ')
        let nodeWeight = int (rawNode.[1].Replace("(","").Replace(")",""))    
        (rawNode.[0].Trim(), nodeWeight)   
           

    type State = { Key : string; Weight : int; Children : State List }

    type Weights = { Cell: int; Cumulative: int }

    type Tree = Node of Weights*Tree list

    let splitLineExp (line:string) = 
        let parts = line.Split([|"->"|], StringSplitOptions.None)
        let rawNode = parts.[0].Split(' ')
        let key = rawNode.[0]
        let nodeWeight = int (rawNode.[1].Replace("(","").Replace(")",""))   
        if parts.Length > 1
        then
            let values = parts.[1].Split(',') |> List.ofArray|> List.map(fun (x:string) -> x.Trim())
            (key, (nodeWeight, values)) 
        else
            (key, (nodeWeight,[])) 


    let SolvePart2Exp (rawInput:string) = 
        let root = "azqje"

        let input = rawInput.Split('\n')
                    |> Array.map (fun line ->  (splitLineExp line))

        let graph = Map.ofSeq input

        let weights (Node (w, _)) = w

        let cumulative (Node (w, _)) = w.Cumulative

        let rec toTree (graph: Map<string, int*(string list)>) nodeName =
          let w, children = Map.find nodeName graph

          let treeChildren = List.map (toTree graph) children

          Node ({ Cell=w; Cumulative=w+List.sumBy cumulative treeChildren }, treeChildren)

        let rec findUnbalances (Node (_, children)) =
          match List.length (List.distinctBy cumulative children) with
            | 2 -> List.map weights children::List.collect findUnbalances children
            | _ -> List.collect findUnbalances children

        printfn "%A" (Seq.last (findUnbalances (toTree graph root)))
        0


    let SolvePart2 (input:string) (root:string) =

        let nodes = input.Split('\n') |> Array.map splitLine2 |> Map.ofArray
        let structure = input.Split('\n') |> Array.map splitLine |> Map.ofArray
        
        let rec getSum node total =
            let subNodes = structure.Item node
            if Array.length subNodes = 0
            then 
                nodes.Item node + total
            else 
                (subNodes |> Array.sumBy(fun x -> getSum x total)) + (nodes.Item node)
                

        let rootItem = structure.Item root
        let totals = 
            rootItem
            |> Array.map(fun x -> getSum x 0)            
            
        let broken = 
            totals
            |> Seq.groupBy(fun x -> x)
            |> Seq.minBy(fun (k, v) -> Seq.length v)
        
        let index = totals
                    |> Array.findIndex (fun x -> x = (fst broken) )            

        let temp = nodes.Item ((structure.Item root).[index])
        ...
        let dif = (fst nonBroke) - (fst broken)
        temp + dif
...
kligtj (80)"
    ...
    let ``Given a input then the correct output is returned`` () = 
        printfn "%A" (Day7App.SolvePart2 problem "azqje")
        Assert.That(Day7App.SolvePart2 problem "azqje", Is.EqualTo(646))

    [<Test>]
    let ``Given a input then the correct output is returned exp`` () = 
        printfn "%A" (Day7App.SolvePart2 problem "azqje")
        Assert.That(Day7App.SolvePart2Exp problem, Is.EqualTo("azqje"))

    [<Test>]
    let ``Given a test input then the correct part 2 output is returned`` () = 
        printfn "%A" (Day7App.SolvePart2 input "tknk")
        Assert.That(Day7App.SolvePart2 input "tknk", Is.EqualTo(60))