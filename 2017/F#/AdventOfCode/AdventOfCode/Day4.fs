...
module Day4App =
    ...
    let SolveExp input = 
        ...
        let sorted = Seq.sort >> Seq.map string >> String.concat ""

        let worded = Array.map (split ' ') input

        worded
        |> Seq.filter(fun words -> Array.length words = Set.count (Set.ofSeq (Seq.map sorted words)))
        |> Seq.length

    let Solve (x:string) = 
        x.Split(' ')
        |> Seq.groupBy (fun i -> i)
        |> Seq.sumBy (fun (i,n) -> if Seq.length n = 1 then 0 else 1)
        |> id = 0

    let SolvePart2 (x:string) =
        x.Split(' ')
        |> Seq.map (fun i -> (Seq.sort i) |> Seq.toArray |> System.String )
        |> Seq.groupBy (fun i -> i)
        |> Seq.sumBy (fun (i,n) -> if Seq.length n = 1 then 0 else 1)
        |> id = 0 

    let SolveArray input = 
        input
        |> Array.sumBy(fun x -> if Solve x = true then 1 else 0) 
        
    let SolvePart2Array input = 
        input
        |> Array.sumBy(fun x -> if SolvePart2 x = true then 1 else 0)    
...
module Day4Tests =
    ...
    let ``Then the input gives the correct output part 2`` input expected =
        Assert.That(Day4App.SolvePart2 input, Is.EqualTo expected )


    [<TestCase("aa bb cc dd ee", true)>]
    [<TestCase("aa bb cc dd aa", false)>]
    [<TestCase("aa bb cc dd aaa", true)>]  
    let ``Then the input gives the correct output`` input expected =
        Assert.That(Day4App.Solve input, Is.EqualTo expected)
...
"xzvfbf fopmfxu mvftgr mfupoxf coyhof talcc vpkslo"|]
    ...
    [<Test>]
    let ``Solve Puzzle`` () = 
        Assert.That(Day4App.SolveArray testInput, Is.EqualTo 451)

    [<Test>]
    let ``Solve Puzzle part 2`` () = 
        Assert.That(Day4App.SolvePart2Array testInput, Is.EqualTo 223)
